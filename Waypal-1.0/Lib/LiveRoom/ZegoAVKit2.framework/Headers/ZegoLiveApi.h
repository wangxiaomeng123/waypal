...
/// \brief 发布质量更新
/// \param quality 推流质量详细数据
/// \param streamID 发布流ID
- (void)onPublishQualityUpdate:(ZegoAPIPublishQuality)quality stream:(NSString *)streamID;

/// \brief 观看质量更新
/// \param quality 拉流质量详细数据
/// \param streamID 观看流ID
- (void)onPlayQualityUpdate:(ZegoAPIPlayQuality)quality stream:(NSString *)streamID;
...
/// \note 不建议使用，请使用 - onPlayQualityUpdate:stream:
...
/// \param fps 帧率(frame rate)
/// \param kbs 码率(bit rate) kb/s
/// \note 不建议使用，请使用 - onPublishQualityUpdate:stream:
